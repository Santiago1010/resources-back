...
trait Validation {
	...
	protected function validateEmail(string $email) : bool {
		return preg_match('/^[A-z0-9\\._-]+@[A-z0-9][A-z0-9-]*(\\.[A-z0-9_-]+)*\\.([A-z]{2,6})$/', $this->setSpecialStrings($email)) ? true : false;
	}

	protected function validateJustString(string $string) : bool {
		return is_string($this->setSpecialStrings($string));
	}

	protected function validateJustNumbers(int $number) : bool {
		return is_numeric($this->setSpecialStrings($number));
	}
